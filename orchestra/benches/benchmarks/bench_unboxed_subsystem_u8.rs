...
fn test_unboxed_u8(c: &mut Criterion) {
	let pool = ThreadPool::new().unwrap();
	c.bench_function("send unboxed message u8", |b| {
		let pool = pool.clone();
		b.iter_custom(|niters| {
			let pool = pool.clone();
			let start = executor::block_on(async move {
				let (orchestra, _handle) = BenchUnboxedU8::builder()
...
criterion_group!(benches, test_unboxed_u8);
